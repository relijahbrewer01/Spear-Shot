...
func _audit_pacing_and_timer_contract(main_scene: PackedScene) -> void:
	...
	_require(
		_is_ambient_enemy_available(main, director, EncounterDirector.EnemyKind.EXPLODER),
		"Exploder is eligible at 65 seconds."
	)
	main.set("survival_time", 64.9)
	_require(
		not _is_ambient_enemy_available(main, director, EncounterDirector.EnemyKind.EXPLODER),
		"Exploder is unavailable before 65 seconds."
	)
	...
	main.set("survival_time", 65.0)
	_require(
		is_equal_approx(float(main.call("_get_current_exploder_spawn_chance")), 0.025),
		"Exploder spawn chance starts at 0.025 at unlock."
	)
	main.set("survival_time", 75.0)
	_require(
		is_equal_approx(float(main.call("_get_current_exploder_spawn_chance")), 0.0285),
		"Exploder spawn chance grows by 0.00035 per second after unlock."
	)
	main.set("survival_time", 1000.0)
	_require(
		is_equal_approx(float(main.call("_get_current_exploder_spawn_chance")), 0.07),
		"Exploder spawn chance caps at 0.07."
	)
	...
	director.reset_for_new_run()
	var exploder := Node.new()
	main.add_child(exploder)
	director.register_enemy(
		exploder,
	...
	_require(
		not director.can_spawn_enemy(EncounterDirector.EnemyKind.EXPLODER, 80.0),
		"Exploder active cap remains one."
	)
...
func _audit_randomized_intro_contract(main_scene: PackedScene) -> void:
	...
	await _audit_exploder_intro_contract(main_scene)
...
func _audit_exploder_intro_contract(main_scene: PackedScene) -> void:
	var main := await _spawn_main_for_intro_audit(main_scene)
	var director := main.get_node("EncounterDirector") as EncounterDirector
	...
	main.call("debug_set_ambient_roll_sequence", [0.0])
	var early_exploder_kind := int(main.call("_pick_ambient_enemy_kind"))
	_require(
		early_exploder_kind == EncounterDirector.EnemyKind.EXPLODER,
		"Exploder can appear randomly after unlock but before its target once earlier special slots are unavailable."
	)
	_require(
		_try_spawn_for_audit(main, early_exploder_kind, SPAWN_SOURCE_AMBIENT),
		"Early random Exploder can be spawned organically."
	)
	_require(bool(main.get("exploder_intro_seen")), "Early organic Exploder cancels its guarantee.")
	await _free_intro_audit_main(main)
	...
	for enemy_kind in [
		...
		main.add_child(placeholder)
		director.register_enemy(placeholder, enemy_kind, EncounterDirector.INVALID_WAVE_ID)
	var pending_exploder_kind := int(main.call("_pick_ambient_enemy_kind"))
	_require(
		pending_exploder_kind == EncounterDirector.EnemyKind.EXPLODER,
		"Pending Exploder intro remains overdue until a successful ambient spawn fulfills it."
	)
	_require(
		not bool(main.get("exploder_intro_seen")),
		"Overdue Exploder intro is still unseen before its organic spawn succeeds."
	)
	await _free_intro_audit_main(main)
