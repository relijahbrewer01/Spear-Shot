...
def main() -> int:
    ...
    require("movement_speed_scale := 0.82" in shooter, "Shooter movement scale starts at 0.82", failures)
    ...
    require("enum ShooterState" in shooter and "LOCKED" in shooter, "Shooter uses explicit attack states", failures)
    require("aim_duration := 0.55" in shooter and "locked_duration := 0.30" in shooter, "Shooter telegraph timing is explicit", failures)
    require("burst_interval := 0.18" in shooter, "Shooter two-dart burst interval starts at 0.18 seconds", failures)
    require("recover_duration := 0.16" in shooter, "Shooter post-burst recovery is short", failures)
    require("post_burst_reposition_duration := 0.35" in shooter, "Shooter relocates after a completed burst", failures)
    require("minimum_dart_interval := 2.4" in shooter, "Shooter has a minimum dart interval", failures)
    require("dart_requested.emit" in shooter, "Shooter asks Main to spawn darts", failures)
    ...
    require("take_damage" in dart and "can_take_damage" in dart, "Dart uses the player damage authority", failures)
    ...
    require(shooter_image.size == (18, 20), "Shooter sprite is reduced to 18x20 pixels", failures)
